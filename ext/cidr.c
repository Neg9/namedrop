...
int cidr_get(const char *instr, struct sockaddr *net_id, struct sockaddr *netmask, unsigned int *cmask) {
	...
	unsigned int maskset=0;
	void *sc=NULL;

	net_u.p=net_id;
	mask_u.p=netmask;

	if (net_id == NULL || netmask == NULL || instr == NULL || cmask == NULL) {
		return -1;
	}
	...
	*cmask=0;
	...
	if (sscanf(instr, "ipv4:%4095[^/]", tbuf) == 1) {
		...
		ERR("cant even get host part of cidr string");
		return -1;
	}
	...
	if ((masksl=strrchr(instr, '/')) != NULL) {
		masksl++;
		if (sscanf(masksl, "%u", cmask) != 1) {
			ERR("bad mask (numeric?)!");
			return -1;
		}
		maskset++;
	}
...
#endif
		mask_u.sin->sin_addr.s_addr=*cmask == 0 ? 0 : htonl(cidrmasktbl[*cmask - 1]);
...
#endif
		cidr_fill6mask(mask_u.sin6->sin6_addr.s6_addr, *cmask);
	...
	sc=stddns_init(NULL, sd_flags);
	if (sc == NULL) {
		return -1;
	}

	sl=stddns_getaddr(sc, tbuf);
	if (sl == NULL) {
		stddns_fini(&sc);
		return -1;
	}
...
int cidr_inchost(struct sockaddr *p, struct sockaddr *n, struct sockaddr *m) {
	union {
		struct sockaddr *p;
		struct sockaddr_in *sin;
		struct sockaddr_in6 *sin6;
