...
sockaddr_list_t **stddns_getaddr(void *c, const char *name) {
	...
	if ((iret=getaddrinfo(name, NULL, &hint, &ret)) != 0) {
		...
		DBG("getaddrinfo fails for %s", name);
		return NULL;
	}
	...
	DBG("got %u awnsers for %s", cnt, name);
	...
	for (idx=0, walk=ret; walk != NULL; walk=walk->ai_next, idx++) {
		...
		if (s->verbose > 4) {
			hexdump((const uint8_t *)walk->ai_addr, 16);
		}

		/* XXX we dont get all the cnames this way */
		sstring=cidr_saddrstr(walk->ai_addr);
		DBG("index %u for name `%s' ai_flags %d ai_family %d ai_socktype %d ai_protocol %d ai_addrlen %zu ai_addr %p (%s) ai_canonname %s ai_next %p\n", idx, name, walk->ai_flags, walk->ai_family, walk->ai_socktype, walk->ai_protocol, walk->ai_addrlen, walk->ai_addr, sstring != NULL ? sstring : "Nothing", walk->ai_canonname != NULL ? walk->ai_canonname : "Null", walk->ai_next);

		if (ename == NULL && s->exact && walk->ai_canonname != NULL) {
			ename=walk->ai_canonname;
			DBG("setting ename to `%s' from `%s'", ename, name);
		}
...
int stddns_getaddr_cb(void *c, const char *name) {
	...
	for (idx=0, walk=ret; walk != NULL; walk=walk->ai_next, idx++) {
		...
		if (ename == NULL && s->exact == 1 && walk->ai_canonname != NULL) {
